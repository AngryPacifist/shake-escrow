...
/// here. Changes reach future wagers only: a live wager carries the snapshot it took at
/// creation.
...
pub fn handle_update_config(ctx: Context<UpdateConfig>, args: UpdateConfigArgs) -> Result<()> {
    ...
    validate_config_values(fee_bps, min_stake, max_stake, resolvers, max_window)?;
    ...
    if let Some(v) = args.max_open_per_wallet {
        config.max_open_per_wallet = v;
    }
    ...
    if let Some(v) = args.max_total_open {
        config.max_total_open = v;
    }
    if let Some(v) = args.new_admin {
        require!(v != Pubkey::default(), ShakeError::NotAdmin);
        config.admin = v;
    }
    Ok(())
}
